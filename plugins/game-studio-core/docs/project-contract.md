...
> **它不回答「这些文件放在哪个仓里」。** 接了工作室服务器的项目是**两个仓**
> （游戏工程仓 + 只装 `plan/ design/ team/` 的文档仓），服务器上还要配名册和
> 写入令牌 —— 那件事在 [`repo-and-deploy.md`](repo-and-deploy.md)。
> **两份分开是有意的**：这一份说「该有什么」，那一份说「摆在哪儿、怎么部署」，
> 合成一份的话，不接服务器的项目会以为自己也得建两个仓。
