...
1. 建 `plugins/<name>/.claude-plugin/plugin.json`（`{"name": "...", "version": "...", "description": "..."}`，
   name 用 kebab-case。**`version` 要写，而且每次改了都要 bump** —— 不写的话那个插件
   回到「commit 即版本」的老模型，跟另外两个不一致；写了不 bump 则各项目静默停在旧版。
   提交时 `validate-git` hook 会替你查这一条）
