...
不管 Lite 还是 Full，开头都要有这四节。第一节是**写给人读的**，
后三节是**约束载体**。

| 节 | 写什么 | 为什么必须在文件里 |
|---|---|---|
| `## 读本` | **一份完整的策划案**，白话，不带数值。规则在 `rules/design-docs.md` 的「读本怎么写」那一节，**动笔前先读那份** | 判据：**不做这块的人只读读本，就该完全懂这个系统怎么运转**。下面那几节是给实现和 QA 的契约 —— 精确、可判定、带数值，而看它们的人得先知道这篇在讲什么。客户端打开一篇 GDD 时**默认停在这一节**；没有这一节，它明说这篇还没有读本 |
...
[它在整个游戏里占什么位置，拿掉它游戏会缺什么。也说清它**不管**什么]

### 玩家在里面经历什么

[从玩家视角走一遍：他看到什么、做什么决定、赢了输了分别是什么感受]

### 它怎么运转

[**最长的一段，而且必须完整**：每条会影响玩家决策的规则都要讲到，
用白话，不写数值和公式。Lite 的取舍理由并进这一段。

长过十来段就用 `####` 分小节，按**玩家在做什么**分；
并列的几种情况写成列表；每段开头一句加粗说完这一段的结论]
...
[长过十来段就分小节 —— 客户端的目录认到四级，分了才跳得过来。
按**玩家在做什么**分（走路和看路 / 碰上敌人 / 跟东西打交道 / 出岔子的时候），
不按实现模块分。
...
[关键取舍。定成另一个样子会怎样、为什么没那么定]

### 一个完整的例子

[从头到尾走一遍具体的一局 / 一次交互，带上具体情境]
