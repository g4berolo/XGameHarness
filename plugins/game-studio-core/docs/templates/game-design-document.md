...
## 读本是**另一个文件**（2026-09-14 搬的）

`design/digest/<跟这篇 GDD 同名>.md` —— **一份完整的策划案**，白话，不带数值。
规则和文件模样在 `rules/design-docs.md` 的「读本怎么写」那一节，**动笔前先读那份**。

判据：**不做这块的人只读读本，就该完全懂这个系统怎么运转。**
下面这份 GDD 是给实现和 QA 的**契约** —— 精确、可判定、带数值。

**为什么分开**：做开发的 agent 读 GDD 是为了规则表，而读本是白话（实测占一篇的 37%），
在一个文件里就会每次都跟着进上下文。客户端把两个文件合起来显示，
打开一篇 GDD **默认停在读本**；没有读本时它明说这篇还没有。

⚠ **读本旧没旧，全靠这篇 GDD 头部的 `Last Updated`** —— 读本里记着它写的时候那一格是什么。
**改了这篇就要改那一格**，否则读本悄悄过期，而且不报错。

## 两档共有的三节（写在最前面）

不管 Lite 还是 Full，开头都要有这三节。它们都是**约束载体**。
...
> 用于命中升级判据的高风险改动。以下 9 段是必需的（共有的三节 + Full 的六节），
> 末尾三段可选。**另外还要有读本那个文件**，见本文件开头。
>
> （这个数原来写的是 8，那时连读本一起数是 10 —— 两边都不对。
> 数一遍：设计前提 · 已定决策摘要 · 本篇用到的新词 · Detailed Design ·
...
# 【读本】模板 —— `design/digest/<跟 GDD 同名>.md`

**单独一个文件**（2026-09-14 搬的，理由在本文件开头那一节）。
九节怎么写、句子怎么写、标题怎么编号、哪些话不许写，全在
`rules/design-docs.md` 的「读本怎么写」那一节 —— **动笔前先读那份**，
这里只给文件的模样。
...
[这个系统管什么、跟哪几个系统有关、玩家在哪儿会碰到它。三到五句]

### 2. 机制构成

[每一条会影响玩家决策的规则。按玩家在做什么分小节]

...（Full 写到第 9 节，Lite 写 1-5 节，节名和写法见规则那一份）
```

**三件事写错了不会有任何地方报错：**

- `写于文档的最后更新` 那一行 —— 客户端拿它判读本旧没旧。写错或者不写，
  那个「读本旧了」的提示**永远不亮**
- **标题一律三级 `###`，从 1 连排**（Full 到 9，Lite 到 5）—— GDD 正文用 `##`，
  读本里再用 `##` 会让客户端的目录多一层空壳
- **不写数值和公式** —— 那是 GDD 那半的事。同一个数写两处，改了一处不会有地方报错
