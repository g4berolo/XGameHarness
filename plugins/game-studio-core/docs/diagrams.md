...
**按读者需要选择 Mermaid 或 SVG。资源变化、动作分镜、前后对照和空间布局优先用 `svg` 图解；复杂关系、调用时序可用 Mermaid。两种都用主题词表上色。**
...
这条是硬的（2026-09-15 加的）：`机制构成` 里凡是在说「谁在谁旁边」「转了多少」
「哪两个能接上」的小节，写完之后回头看一眼 —— 没有图解就是漏了。
⚠ **这类小节画成 mermaid 是白画**：它排的是关系，把位置这件事正好画没了。

**图帮助读者看懂变化，文字保留条件、例外和完整解释。** 不以少字为目标，
不要求加图后必须删文字。图和文字可以互相印证，但不能新增或改变规则。
...
- **空间布局**（房间怎么摆、门在哪、关卡形状、界面长什么样）→ mermaid 画不了自由图形，
  用 **` ```svg ` 图解**（见下面「图解」那一节）
- **资源变化、动作结果、前后对照** → 优先用 SVG 画出对象本身：体力条、角色动作、容量轮廓等。
  不把 SVG 限定为空间图，也不把 Mermaid 方框原样描一遍当作美化。
...
资源如何变化、动作有什么后果、前后有什么差别，也可以用 SVG。
多方依赖和调用时序需要精确连线时用 Mermaid；不因系统没有空间内容就排除 SVG。
...
先读相应 GDD，选一个读者需要理解的问题，再安排画面。机制图的构图示例见
[`diagram-examples/mechanism.md`](diagram-examples/mechanism.md)。示例只示范画法，不提供游戏规则。

- 用稳定的轮廓作参照，用填充、位置或姿态表现变化；前后图保持同一尺度。
- 画出主题对象，不只堆标题卡片。每个分镜写动作或条件，以及可见结果。
- 保持边距、对齐与留白；标签靠近对应对象，连线避开文字。语义色和文字标记共同表达差别。
- 不为了好看加入源文档没有的阈值、等待时间、比例或因果。体力条长度只作示意时标明“示意，非数值比例”。
- 保留解释所需文字，不强求短标签或固定图数。内容放不下时拆图，不缩小字体硬塞。
- 生成后在客户端查看明暗两档，确认字可读、标签不重叠、箭头指向正确、图未被裁切。
  无法渲染验证时明确标为待视觉验收，不以语法正确代替观感验收。
...
7. **一张图只解释一个问题。** 位置、资源和动作可以为同一个问题服务；无关问题拆开，按内容选择 SVG 或 Mermaid。
...
| 文档 | 最该画的 |
|---|---|
| **GDD 读本** | SVG 机制图（资源变化、动作分镜、前后对照、空间布局）；Mermaid 关系与时序图 |
