...
参数：<system-name> (e.g., 'combat-system', 'inventory', 'dialogue')
...
...（Full 九节、Lite 五节，节名和写法见 rules/design-docs.md 的「读本怎么写」；
    Section A 写第 1 节，Section H 在契约写完之后补完剩下的）
```
...
### Applies to both rigor levels: 不编规则

**GDD 里每一条规则，都要能指回三处之一：** `## 设计前提`、`## 已定决策摘要` 里
某个上游文件、用户在这次对话里的回答。三处都指不回的，**不许写成规范**。

觉得缺一条的时候，二选一：

- 用用户已经用过的词问一句是非题，他答了再写
- 写进 `## Open Questions`，标「建议」，让他以后决定

下面各节的「要问的问题」里，「玩家会怎么钻空子」「零值 / 最大值 / 负值会怎样」
这类是**提醒你去问用户**，不是让你自己答完写进去。每堵一个空子就是一条新规则，
而新规则会长出新的边界情况，再长出新的旋钮 —— 一篇 GDD 就是这么从六句话涨到五百行的。

> **实测（2026-09-15，读本实验室 r01）**：设计前提六句话，写出来的 GDD 有 533 行。
> 贴墙起手扣一口、爬分「挪动 / 挂着」两档速率、喘息线、缩圈、扩容硬帽、
> 游比跑贵、走路也回体力、不做永久成长 —— **全是 agent 自己加的**，
> 用户一条都没说过。他的原话：
>
> > 自己过多思考和添加是导致不简洁和规则冲突的元凶。
>
> 更要紧的是：自己编的规则**没有任何地方会核对它**。r01 写「送回下水的那一岸」，
> r02 同样的前提写「送回最近的岸」，两次正好相反，而两次都没报错。
...
### Section A: 读本的第 1 节「概括」

> **`Overview` 和 `Player Fantasy` 这两节没有了**（规则 2026-09-07 改的）——
> 它们说的正是读本第 1 节的内容。留着的话同一个系统会被写两遍，
> 而**两处说法走散时不会有任何地方报错**。
>
> 要问的问题一个没少，只是答案写进读本里。
>
> ⚠ **读本是另一个文件**：`design/digest/<跟 GDD 同名>.md`（2026-09-14 改的）。
> 这一节和 Section H 都写到那个文件里，**不写进 GDD**。

**写什么**：读本的第 1 节，标题是 `### 1. 概括`。**三到五句**：
这个系统管什么、跟哪几个系统有关、玩家在哪儿会碰到它。
...
**剩下几节等设计完了再写**（Section H）：`机制构成` 要覆盖每一条会影响
玩家决策的规则，而那些规则这会儿还不存在。

**句子怎么写、节名是什么、哪些话不许写，全在 `rules/design-docs.md` 的
「读本怎么写」那一节 —— 动笔前先读那一份。** 最容易忘的三条：
一句话只说一件事不超过 30 字、不打比方不用破折号、正文不加粗。
...
### Section H: 读本剩下那几节（**最后写，写进读本那个文件**）

到这儿契约那几节都写完了 —— 现在才有东西可以用白话讲完整。

⚠ **写到 `design/digest/<跟 GDD 同名>.md`**，接在 Section A 那一节后面。
...
**写什么**：Full 补第 2 到 9 节，Lite 补第 2 到 5 节。
**节名、句子规矩、标题规矩、三条硬规矩，全在 `rules/design-docs.md` 的
「读本怎么写」那一节，动笔前先读那一份，这里不复述。**

**动笔前把 GDD 上面那几节整篇读一遍。** 规则散在 Detailed Design、Edge Cases、
Tuning Knobs 里，漏一条的表现是**读本看起来很完整、但少讲了一件事** ——
没有任何地方会报错。

写的时候盯住三条，它们是这一步最容易坏的地方：

1. **一句话只说一件事，不超过 30 字。** 写「普通行走不消耗体力，并且缓慢回复」，
   不写「走路是免费的，而且还在回。这是整套规则的地基」
2. **`机制构成` 那一节只写规则，不写理由和评论。** 「这是故意的」「否则会怎样」
   放 `关键决定` 那一节，或者不写
3. **`关键决定` 和 `容易搞错的地方` 只写 GDD 里已经有的。**
   GDD 里没有的取舍，读本不许补一条出来

**`机制构成` 至少要有一张图，Lite 也要。** 判据是**读者只扫标题和图，
就该知道这个系统大致怎么转**：有状态就画流转图，一件事有几种去向就画分支图，
资源变化、动作分镜、前后对照和空间布局优先用 SVG，关系与调用时序可用 Mermaid。
保留解释所需文字；完整规矩在 `rules/design-docs.md` 的「图」那一节，构图见 `docs/diagrams.md`。
⚠ 反过来别硬凑 —— 一张图要讲一个变化或一组对比，不是罗列。
