...
关系和流转用 mermaid，空间用 ```svg 图解。**空间那种画成 mermaid 是白画** ——
它排的是关系，把位置这件事正好画没了。骨架、绘图词表和语法坑在 `docs/diagrams.md`。
