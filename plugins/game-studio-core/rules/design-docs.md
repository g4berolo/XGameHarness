...
4. **玩家在里面经历什么** —— 从玩家视角走一遍：他看到什么、做什么决定、
   ...
   > **纯基础设施系统例外。** 有些系统的设计目标就是**玩家没有体验** ——
   > 没有可见界面、验收条件写着「玩家不可分辨」。这一节标题预设了
   > 「有个里面可以进去」，对它们不成立。
   >
   > 这时**照写这一节，但写成一两句**：说清玩家感知到的是什么
   > （通常是下游系统的体验），以及**为什么这个系统本身不该被感知到**。
   > **不要硬凑** —— 把下游的玩家体验整段搬过来充数，比写两句诚实的话更糟。
...
7. **N 个关键决定** —— 关键取舍，**逐条写**，每条一个四级标题：
   这个决定是什么、**定成另一个样子会怎样**、为什么没那么定。
   写成一段连排的「权衡讨论」等于没写 —— 读者记不住，也无法逐条引用

   > 标题里那个 N 是实数，`### 7. 八个关键决定`。
   > **不要写成「为什么这么定：八个关键决定」** —— 序号点加冒号，
   > 客户端目录里会拖成一长条。这一节和第 8 节都把数目直接嵌进标题，形态一致。
...
10. **名词对照** —— 白话说法 ↔ 文档/代码里的真实名字，一张表。
    ...
    > **合规的源文档自己没有真名可给。** GDD 正文里本来就不该有类名和字段清单，
    > 所以这张表的真名多半得去上游文档、ADR 或代码里取 ——
    > 这是「上游只为搞懂、不要抄进来」的唯一例外。
    > 只取读者**对接时一定会碰到**的那些；**取不准的宁可不写**，
    > 或标「（待核）」。错的映射比没有映射更糟。
...
> 中途把一个小节拆成两个、或者删掉一条，后面全得跟着动 ——
> 漏排一个不会有任何地方报错，而症状恰好是「目录里数不出总数」，
> 也就是编号本来要解决的那个问题。
>
> 核法：把所有 `###` / `####` 标题抓出来扫一遍，确认
> 三级从 1 连到末节、每节四级都从 `.1` 起连排、写了数目的标题子项数对得上。
>
> **数目那一条要核到图里。** 实测踩到过：四级标题写「一次拼装只有三种结局」，
> 底下的分支图画了四个结果框。既然「该画就画」那条鼓励用图替掉列表，
> **标题-图数目错位会经常发生** —— 数的是**读者看到的条目**：
> 列表项、表格行、图里的终结点，哪种承载的就数哪种。

**正文里引用别的小节，只用粗粒度。** 写「见第 7 节第 1 条」，
不要写「见 7.1」。**读本是会被重排的** —— 粗粒度引用重排后还救得回来，
精确引用全是悬空的。
...
- **父标题里写上数目**。`最容易搞错的七件事`、`八个关键决定` ——
  读者一眼知道这一节有多少条、还剩几条。
  数目**直接嵌进标题**，不要用冒号外挂（`为什么这么定：八个关键决定` ❌），
  序号点再加冒号会在目录里拖成一长条。

  > ⚠ **数目和条数是双向锁死的。** 定了「九件事」，第九条就不能再删 ——
  > 要删得同时改标题。这不是坏事（它逼你把条目写扎实），但得知道：
  > 写不下去的时候**换掉那一条的内容**，别直接删条目。
...
#### 四、该画就画 —— 关系用 mermaid，空间用 ```svg 图解

**判据：一段话要读者在脑子里同时记住三样以上东西之间的关系、
或者在脑子里「摆」出一个形状时，把它画出来。**

- ✅ 该画：「三种拼接失败都走兜底，而五种真错误根本不产生战场」
  —— 两组东西、两条去向，读者得同时拎住（**关系 → mermaid**）
- ✅ 该画：「锚房落在原点，东边那间不转，北边那间转了半圈」
  —— 读者得摆出一个 L 形（**空间 → ```svg 图解**）
- ❌ 不该画：「容差是半格」—— 一句话说完的事，画出来是装饰

骨架、绘图词表、语法坑和完整规矩在 `docs/diagrams.md` —— 那是 harness 的单一出处，
GDD / ADR / 便条共用。这里只说读本相关的几条。
...
**图解**（房间怎么摆、门在哪、有哪几种朝向）写在 ` ```svg ` 围栏里，
形状挂**绘图词表**里的名字（`box` `gap` `route` + `key` `ok` `no` `warn`），
骨架照抄 `docs/diagrams.md` 的「图解」那一节。

**两种画法都不写颜色。** 要上色就用名字：mermaid 写 `class C ok`，
图解写 `class="box ok"`。`style C fill:#…`、`fill="#…"`、`<style>` 客户端一律不用 ——
写死的浅绿在深色档下是浅底浅字，看不清。

**矩阵、对照、参数表用 GFM 表格**，不要用图硬画。

**围栏外面的原始 HTML 绝对不能用**（包括直接写进正文的 `<svg>`）——
客户端故意不装 `rehype-raw`（文档是不可信输入），写了会被转义成一屏尖括号，
不报错，就是没法读。图解只能写在 ` ```svg ` 围栏里。

**图里照样不许写数值**，跟正文一个标准 —— 那条规矩管内容，跟画不画无关。
图解里写「按撞见敌人的那间算位置」，不写「(6000, 0)」。
