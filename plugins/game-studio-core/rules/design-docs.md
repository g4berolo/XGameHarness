...
|---|---|
| **GFM 表格** | ✅ 渲染最好，矩阵类首选 |
| **ASCII 框线图**（在代码围栏里） | ✅ 能显示，流转/分支/时序都能画 |
| 内联 `<svg>` / 任何原始 HTML | ❌ **被转义成文本**，读者看到一屏尖括号 |
| ` ```mermaid ` 围栏 | ❌ 渲染成代码块，读者看到 mermaid 源码 |

> **为什么后两种不行 —— 写在这儿是为了让你别再试一次。**
>
> 客户端的渲染栈是 `react-markdown` + `remark-gfm`，没有别的。
> 它**故意不装 `rehype-raw`**（`Markdown.tsx` 第 12 行有明确注释）：
> 不装它，markdown 里的原始 HTML 就会被转义成文本而不是执行。
> 理由是 **agent 的输出是不可信输入**，这一层不能松。
>
> mermaid 则是压根没装 —— 全仓零篇 GDD 用过它，
> 不是没人想用，是用了也看不见。

**ASCII 图里照样不许写数值** —— 画结构，不画数字。那条规矩管内容，跟画不画无关。
...
拼装请求
├── 几何对得上 ────────▶ 正常战场
├── 对不上（三种） ────▶ 兜底战场（能打，玩家看不出）
└── 前提就不成立 ──────▶ 没有战场（中止，不消费触敌）
```
...
**空间布局是 ASCII 最吃力的一类**（房间怎么摆、L 形长什么样）。
这类如果值得画好，做一个配套的 HTML 图解页，读本里用一行链接指过去 ——
别在 ASCII 里硬画，画出来比不画还难读。
