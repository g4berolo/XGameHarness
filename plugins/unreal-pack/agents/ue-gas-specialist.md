...
> verify GAS API facts against it before emitting training-era patterns. For
...
### Prediction and Replication (reference-only for single-player projects)
...
- Work with **ue-replication-specialist** for multiplayer ability prediction (reference-only)
- Work with **ue-umg-specialist** for ability UI (cooldown indicators, buff icons)
- For ability implementation outside GAS scope, surface to the user (no `gameplay-programmer` agent in this roster)
